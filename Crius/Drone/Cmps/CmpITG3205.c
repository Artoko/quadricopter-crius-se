...
Boolean CmpITG3205Init(void)
{
	...
	if(ITG3205_RA_I_AM == datum)
	{
		...
		DrvTimerDelayUs(200);
		DrvTwiWriteReg(ITG3205_TWI_ADDRESS, ITG3205_RA_DLPF_FS, FS_RANGE_2000 | LPFBW_42HZ );
		DrvTimerDelayUs(200);
...
Boolean CmpITG3205GetRotation(S_Gyr_Angle *rot)
{
	...
	else
	{
		...
		if(loop_calibration_itg3205 > 0U)
		{
			...
			{
				gyro_calib_itg3205[0U] += rot->x;
				gyro_calib_itg3205[1U] += rot->y;
				gyro_calib_itg3205[2U] += rot->z;
			}
		...
		else
		{
			...
			rot->x  -= gyro_calib_itg3205[0U];
			rot->y  -= gyro_calib_itg3205[1U];
